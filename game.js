...
document.querySelectorAll('.skin-btn').forEach(button => {
    button.addEventListener('click', () => {
        if (!button.disabled) {
            selectSkin(button.getAttribute('data-color'), button);
        }
    });
});
...
function checkSkins() {
    document.querySelectorAll('.skin-btn').forEach(button => {
        ...
        if (highScore >= scoreNeeded) {
            ...
            button.textContent = button.getAttribute('data-color').toUpperCase().replace('#', '') + " (Frei)";
        } else {
            button.textContent = `(${scoreNeeded}P)`;
        }
    });
}
...
function createParticles(x, y, color, count) {
    for (let i = 0; i < count; i++) {
        particles.push(new Particle(x, y, color, (Math.random() - 0.5) * 4, (Math.random() - 0.5) * 4));
    }
}
